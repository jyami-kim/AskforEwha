...
module.exports = new LocalStrategy({
		...
		var parmaemail = req.body.email || req.query.email;
	 
		console.log('passport의 local-signup 호출됨 : '+loginid +', ' + parmaemail + ', ' + password + ', ' + paramName);
	    ...
	    process.nextTick(function() {
		    ...
		    database.UserModel.findOne({ 'loginid' :  loginid }, function(err, user) {
		        ...
		        if (user) {
		        	...
		        	// 모델 인스턴스 객체 만들어 저장
		        	var user = new database.UserModel({'loginid':loginid, 'password':password, 'name':paramName, 'nickname':paramNickName, "email":parmaemail});
